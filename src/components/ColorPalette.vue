<template>
    ...
    <footer class="row-span-1 flex items-center justify-between bg-header-footer px-6 dark:bg-slate-800">
        ...
        <div class="flex content-end items-center space-x-4">
            <!-- <button @click="randomColors"><Icon icon="ph:dice-five-bold" class="text-3xl" /></button> -->
            <p>HSL</p>
            <input
...
const randomColors = () => {
    for (let i = 1; i <= 5; i++) {
        useStorage(`palette${props.selectedPalette}-card${i}-slider1`, (Math.random() * (360 - 0) + 0).toFixed(1));

        useStorage(`palette${props.selectedPalette}-card${i}-slider2`, (Math.random() * (100 - 50) + 50).toFixed(1));
    }
};
</script>
